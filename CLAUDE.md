...
**0.1.0 plan — reassessed.** Architecture is settled; the remaining gates are about whether the app *rewards* a tester's hour. Identified gap: no meaningful plugins out of the box, which turns the app into a multitrack recorder for a friend without a plugin library. Fix: a first-launch install pack of 20 curated free AU plugins. Design captured in `docs/BUNDLED_PLUGINS.md`. At least one more "major feature" (TBD) slated before ship.
