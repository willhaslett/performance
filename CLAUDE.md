...
1. **Merge `action-instances-refactor` to main** (note: `state.db` reset required because the old-format action schemas won't parse as typed — pre-beta policy).
2. **Action-algebra refactor** (~2 focused days). Replace the hardcoded action-dispatch ladder (`if (name == "fadeOut") ...`) with a six-primitive algebra (`Set` / `Interpolate` / `Delay` / `Parallel` / `Sequence` / `Invoke`) + a tiny tree interpreter. Built-in actions become labeled templates with algebra tree bodies; the dispatch ladder disappears; bounce + real-time run the same interpreter against different clocks (resolves the "automation freezes during render" caveat). `Lua` stays as a seventh escape-hatch op for genuinely procedural needs. Full design + 9-step build sequence in `docs/ACTION_ALGEBRA.md`. Lands before 0.1.0 because it's much easier to change the action-execution shape before testers have any bindings saved.
3. **perfuce.com rebuild** (several days). Includes the tester onboarding copy carry-over — example prompts, "chat is free for testers" line, no key-paste instructions. Mostly gated on video capture.
4. **Distribution proof** (second-machine install, ~1 hour). Penultimate step before ship.
5. **Tag + release.**
