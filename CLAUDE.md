...
- `docs/AI_COMPOSITION_API.md` — **active sub-sub-project.** ABC notation + content CRUD redesign. On dev branch `composition-abc`. Read before touching the composer pipeline, `compose()`, `V2NotationParser`, or `runtime/SYSTEM_PROMPT.md` composing section.
- `docs/UNIFIED_EVENTS.md` — **active sub-sub-sub-project.** Sample-accurate audio-thread tempo handling + collapse tempo/timesig/key into the unified action-event vector. Engine refactor that came out of trying to land tempo events in the AI composition push. Same branch (`composition-abc`); blocks the composition merge.
...
**Current sub-project: improving AI composition.** See `docs/AI_COMPOSITION.md` (research + landing) and `docs/AI_COMPOSITION_API.md` (build plan). The current chat-based composition flow ships and works mechanically, but the music it produces is "childish and sometimes plainly bad" (Will's words). Research phase complete; direction committed to **ABC notation + content CRUD layer** (replace V2 notation, expose project / track / region as ABC, add tempo / time-sig / key as first-class CRUD resources). Work happens on dev branch `composition-abc`; merge to main when end-to-end is working. No hurry — wait-and-see phase of friend ship continues. Detours for small fixes / features will happen on `main` as feedback arrives.

**Currently paused at:** mid-Phase-7 testing of `composition-abc`. Tempo events landed mechanically but couldn't be made to play back accurately without a real engine refactor. **Sub-sub-sub-project opened: `docs/UNIFIED_EVENTS.md`** — sample-accurate audio-thread tempo handling + collapse the typed tempo/timesig/key event vectors into the unified action-event vector (Will's "events are instances of actions" framing). Composition merge waits until this engine work ships. Pre-merge blockers from earlier (synth-track / track-preset, see `memory/project_track_preset_first.md`) also still open.
