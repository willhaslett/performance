...
1. **Action-instances refactor** (~1–2 focused days). Typed `ParamSchema` with five primitives (`channelRef`, `presetRef`, `enum`, `float`, `morph`), one unified `ActionInstanceForm` replacing the three divergent creation UIs, cascade-delete with confirmation for track/bus/preset deletions that have dependent action events or bindings, load-time repair dialog for stale refs. Fail-hard stance preserved — no action-handler early-returns. Triggered by a crash where a `crossfade` binding had a blank `toTrack` UUID because the creation UI didn't validate. Full design + step sequence in `docs/ACTION_INSTANCES_REFACTOR.md`.
