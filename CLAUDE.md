...
- **Audio region playback breaks when output device sample rate changes (Bluetooth).** Reported 2026-07-01: selecting Bluetooth headphones for output makes recorded/imported audio play at the wrong speed / "all to hell." Fine on a fresh reset without headphones. Root cause: `AudioFileNode` reads audio at the take's *stored* sample rate (`fileSampleRate`) with **no runtime resampling to the current output device rate** — it re-anchors file position from the beat each block but copies file samples ~1:1 into the output. Audio was recorded/imported at the built-in device rate (48k); Bluetooth forces a different output rate (44.1k for A2DP, 16k for hands-free/SCO), so the playback-rate ratio is off and the audio garbles. `rebuildGraph()` re-preps the graph at the new device rate on device switch, but the node's file→output rate conversion doesn't exist. Same gap flagged during the import design (imports at a non-device rate would mis-play); Bluetooth makes it trivial to hit even for content that imported/recorded fine. Fix: give `AudioFileNode` a resampler keyed on `fileSampleRate / deviceRate`, re-prepared on `prepareToPlay` / device change. Serious for a live app — you can't switch to Bluetooth mid-session. Not yet root-caused in code beyond this hypothesis; verify before fixing.
