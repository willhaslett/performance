...
- Stuck note prevention — three cases: (1) recording stop: coordinator already injects synthetic noteOffs for open notes. (2) Playback stop/seek: GraphWrapper should track which noteOns it has sent and flush noteOffs (or CC 123 all-notes-off) on position change. (3) Region boundary: `scanMidiEvents` should fire synthetic noteOffs at region end for unclosed notes. TODOs marked in Arrangement.cpp.
- Arrangement persistence — regions and MIDI events not yet saved to SQLite. Need `regions` and `region_events` tables in PersistenceLayer, saved/loaded with song.
