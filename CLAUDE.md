...
- `docs/LATENCY_TESTING.md` — **active, in-flight.** Empirical recording-latency data-gathering (built-in vs external device, Performance vs Audacity). Hypothesis-free; no code changes. See the built-in-mic-latency status block below for context.
...
**Reopened 2026-06-13 for empirical data-gathering** (branch `latency-testing`, doc `docs/LATENCY_TESTING.md`). Deliberately hypothesis-free this round — the 2026-05-04 findings above were diagnostic reads, not measured acoustic round-trips, so we're re-establishing the facts from scratch: is the delay real, how big, in which scenarios, and — the pay dirt — is it present in a non-Apple app (Audacity) too, or is it us? Method: 2×2 (app × device) acoustic round-trip via Performance's metronome → speakers → room mic → compare recorded onset to song grid. **No code changes this phase.** Current state: rig + doc set up; **waiting on Will to capture the first recordings** (Performance / built-in cell first). Claude analyzes the WAVs and keeps `LATENCY_TESTING.md` current; how many trials per cell is a decision made from the data, not up front.
...
- **Deleting the focused track → permanent boot-loop crash. (Found 2026-06-13, dogfooding.)** Repro: focus a track, delete it. The delete path does NOT clear `songs.focused_track_id`, so it dangles to a now-gone track ID. On next load the focus path resolves it through the fail-loud `StateAPI::track()` helper (`StateAPI.cpp:68`) → `FATAL: Track not found` → abort. **Amplifier (the worse half):** the `applicationCrashHandler` then runs `coordinator->save()`, re-persisting the bad state — so a one-time crash becomes a *permanent boot loop*; only a state wipe recovers. Two fixes: (1) clear `focusedTrackId` (and any `selectedTrackIds`) on track delete when they point at the deleted track; (2) harden the crash handler / load path so an emergency-save during a *load/boot* crash can't re-poison the DB (e.g. guard emergency save behind "fully booted", or make load resilient to dangling focus/selection/binding refs by clearing them instead of asserting). Repro DB stashed at `/tmp/performance-crash-repro-focus-danglingref.db`. High severity — a delete-causes-unbootable-app is unacceptable for a live tool.
- **`bin/reset` doc is stale in CLAUDE.md.** The "Reset script" section above describes a surgical `sqlite3` DELETE that preserves `keys/`, `plugin-cache.xml`, AND the `devices`/`device_controls` rows. The actual `bin/reset` is a blanket `rm -rf` of `~/.config/performance` preserving only `keys/` and `plugin-cache.xml` — so MIDI device pairings ARE wiped and must be re-paired after every reset. Either fix the doc to match the script, or restore the device-preserving behavior the doc promises (re-pairing hardware every reset is tedious in testing — that was the doc's stated rationale). Found 2026-06-13.
