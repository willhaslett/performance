...
- ⬜ **4. Output async SRC in the device callback** — replace `AudioProcessorPlayer` with a callback that runs the graph at engine rate and converts to the device rate via `DeviceRateBridge` when they differ (the engine→device half). **The real-time commitment.** Fixes SCO Bluetooth (device physically can't run at the engine rate). Click-test SCO Bluetooth.
- ⬜ **5. Input SRC + store-at-engine-rate** — record path converts device→engine; import retargets to engine rate; commit math updated. Fixes existing-content-mis-plays-on-rate-change.
- ⬜ **6. Bounce at engine rate + latency compensation** — `OfflineRenderer` rate pin (bounce currently follows device rate → 16k export on SCO, a separate confirmed bug at `OfflineRenderer.cpp:39`); input/output latency comp on record commit; fold in the built-in-mic investigation below.

**Known caveats until phases 4–6 land** (all by design): **SCO Bluetooth** (forced 16k, device can't run at the engine rate) still mis-plays — needs the phase-4 device-callback SRC; **bounce** still follows the device rate (`OfflineRenderer.cpp:39`) — phase 6. Mixed-rate *playback* (any stored rate at any engine rate) now works via 3b. Full suite green at 282; app builds.
