...
**0.1.0 plan — reassessing.** Now that the architecture is settled, the question moves from "is the code ready?" to "is there enough *there* for a friend to sit with for an hour and have a rewarding time?" Treating the friends round as a one-time opportunity means not spending it if the app isn't ready to reward a real session. Discuss before locking the remaining sequence. Previously listed items that remain in play:

1. **perfuce.com rebuild** (several days). Includes the tester onboarding copy — example prompts, "chat is free for testers" line, no key-paste instructions. Mostly gated on video capture.
2. **Distribution proof** (second-machine install, ~1 hour). Penultimate step before ship.
3. **Tag + release.**
