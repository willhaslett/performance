...
**0.1.0 plan — reassessed.** Architecture is settled; the remaining gates are about whether the app *rewards* a tester's hour. The identified "empty box" gap — no meaningful plugins for a tester without a plugin library — is closed by the bundled-plugin install pack (shipped 2026-04-20; see `docs/BUNDLED_PLUGINS.md`). Composer integration (in-app "Claude writes MIDI") is the one remaining "major feature" before ship.
