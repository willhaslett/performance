...
- ✅ **3. Request engine rate at the device + Settings control** — `AudioEngine::setEngineSampleRate`/`applyEngineRate` (requests rate via `setAudioDeviceSetup`; graph still preps at the device's *actual* rate → safe fallback to device rate when refused; re-asserted on device change). Coordinator drives it from `syncTempoFromState`. Settings "Sample Rate" now sets the per-project engine rate. **Click-test pending** (Will testing: no-regression at 48k; rate-follow on a new project via Settings; persistence).
- ⬜ **4. Output async SRC in the device callback** — replace `AudioProcessorPlayer` with a callback that runs the graph at engine rate and converts to the device rate via `DeviceRateBridge` when they differ. **The real-time commitment.** Fixes SCO + rate-mismatch playback. Click-test SCO Bluetooth.
...
**Known caveats until phases 4–5 land** (all by design, documented): changing a project's rate when it *already has* audio at the old rate mis-plays (no per-file SRC yet, `AudioFileNode` still copies 1:1); SCO Bluetooth (forced 16k) still mis-plays; bounce still follows the device rate. New content recorded/imported at the chosen rate is correct. Full suite green at 279; app builds.
