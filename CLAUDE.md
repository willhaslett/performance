...
- `docs/AI_COMPOSITION.md` — **active sub-project.** Improving the *quality* of what the AI composition flow produces. Research catalog + "where we landed and why" record.
- `docs/AI_COMPOSITION_API.md` — **active sub-sub-project.** ABC notation + content CRUD redesign. On dev branch `composition-abc-clean` (replaces the earlier `composition-abc` which grew an audio-thread tempo refactor that didn't land). Read before touching the composer pipeline, ABC parser/writer, RegionContent, or `runtime/SYSTEM_PROMPT.md` composing section.
- `docs/DAW_BRIDGE_PLAN.md` — forward-looking DAW bridge design.
- `docs/AUDIO_DEVICE_BOUNDARY.md` — **active sub-project, in-flight on branch `audio-device-boundary`.** Fixed per-project engine sample rate + explicit device-boundary SRC (the "how Logic does it" architecture). Locked decisions + 6-phase plan + test-engineering strategy. Read before touching `AudioEngine` device setup, `AudioFileNode`, the record/import commit rates, `OfflineRenderer` rate, or `SongState.sampleRate`. Phases 1–3 landed; see the "Engine sample-rate / device-boundary refactor" note under the release plan for the live status.
...
- `docs/LIVE_INPUT_AND_FOCUS.md` — **Living doc, in-flight.** Per-track live MIDI routing, singular focused-track concept, looper redesign around armed-set + global record. Phases 1–5 landed; phase 6 (Boss-style looper) functionally complete for both MIDI and audio: state-model layer, coordinator gestures (replace/overdub/undo/redo/clear), bootstrap mode, reset session, in-flight note + waveform rendering during capture, real audio overdub via WAV sum-mix. Looper top bar is now a single segmented `BindableButton` strip with per-button trigger-slot binding (replaces the earlier MIDI Learn pill). Load-bearing for live MIDI dispatch, track `R`/`I` semantics, and looper recording flow.
...
**Current version: `0.0.5`** — SSOT is `CMakeLists.txt` line 2: `project(Performance VERSION 0.0.5)`. `0.1.0` will be the first beta. `0.0.5` adds Sara's first-machine fixes on top of `0.0.4`: narrow right-pane default (~220px so Producer's transport bar lays out fully on small laptops), responsive LCD that snaps to compact (BPM + TIME SIG only) or hidden when narrow, and "Open last project on startup" defaulting ON. `0.0.4` was the first build shipped to a tester (Sara) — adds chat history persistence, GM defaults + drum-token enforcement in the compose prompt, BindableButton trigger-slot menu (with shared track-context-menu), Sidebar Track Presets section, MIDI Monitor pane (View menu), and the prompt-caching + 5x cap bumps in the chat Lambda. `0.0.3` was the bundled-plugin install pack baseline.
...
**Build identity:** there is no top toolbar — that real estate was reclaimed for pane content. Build info (commit hash, optional tag) renders centered along the bottom edge of the sidebar in `MainLayout` as a selectable/copyable TextEditor (`textDim`/`fontSizeSm`); hidden when the sidebar collapses. Commit hash is regenerated on every build via `cmake/GenerateBuildVersion.cmake` → `build/generated/BuildVersion.h`, and auto-appends `-dirty` when there are uncommitted changes. No reconfigure needed to refresh it. Every log file also records `[App] Build version=... commit=... tag=... dirty=...` on startup.
...
As of 2026-05-03, the architectural foundation is settled, both reward features (bundled plugins, composer) shipped, and the **looper is functionally complete for both MIDI and audio**. Boss-RC flow holds end-to-end: reset → focus track → tap replace → play → second tap sets cycle length → focus next → tap. Per-track gesture queue, undo/redo (10 deep), reset session, in-flight note + waveform rendering during capture, real audio overdub via WAV sum-mix.
...
**Friend round one — shipped 2026-05-04.** `0.0.5` DMG sent to Chris, Joe, Alec, and Charlie. Pre-ship gate (bundled plugin pack install verification) was confirmed working on Sara's machine before sending. Now in wait-and-see mode.

**Tester feedback log:**
- *2026-05-08:* one tester (after installing the bundled plugin pack) asked the chat "what plugins are installed?" and got "none." When then asked to *use* one of those plugins, chat had no problem doing so. Pure LLM-answers-from-memory bug, not a plugin-loading bug. Fixed by adding a "never answer state-introspection questions from memory — always query first" rule to `runtime/SYSTEM_PROMPT.md` (commit forthcoming). Worth keeping an eye on whether other questions ("what tracks do I have?", "what's bound to my pedal?") show similar failure modes — if so, the rule may need re-emphasis or a different framing.

**Current sub-project: improving AI composition.** See `docs/AI_COMPOSITION.md` (research + landing) and `docs/AI_COMPOSITION_API.md` (build plan). The current chat-based composition flow ships and works mechanically, but the music it produces is "childish and sometimes plainly bad" (Will's words). Research phase complete; direction committed to **ABC notation + content CRUD layer** (replace V2 notation, expose project / track / region as ABC, add tempo / time-sig / key as first-class CRUD resources via the unified `createEvent` API). Work on dev branch `composition-abc-clean` — replaces earlier `composition-abc` after audio-thread tempo refactor didn't land. Merge to main when remaining pre-merge blockers are addressed (synth-track / track-preset, see `memory/project_track_preset_first.md`). Mid-piece tempo *playback* is deferred to a separate engine sub-project after merge — events are data-faithful but inert during transport. No hurry — wait-and-see phase of friend ship continues.

**Current sub-project: engine sample-rate / device-boundary refactor.** In-flight on dev branch `audio-device-boundary` (pushed to origin). Full design + risk + phase plan in `docs/AUDIO_DEVICE_BOUNDARY.md` — read it before continuing. Triggered by the Bluetooth wrong-rate bug (recorded/imported audio garbles when output switches to a device at a different rate). We chose the full architecture (per-project fixed engine rate + explicit device-boundary SRC) over the tactical file-boundary resample, on the grounds that it's a "when not if" refactor and, gated by strong tests + a click-test matrix on a throwaway branch, the risk is "don't merge if it falls on its face."

**Locked decisions:** per-project engine rate (`SongState.sampleRate`, no migration but an `ALTER TABLE` was added for dev-DB continuity); store recorded/imported audio *at* the engine rate; resampler = JUCE `WindowedSinc` + ring buffer (no hand-rolled DSP); fast path requests the engine rate from the device (`setAudioDeviceSetup`) and only runs the async SRC when the device refuses (SCO-class); bounce pinned to the engine rate.

**Phase status (6-phase plan in the doc):**
- ✅ **1. Per-project engine rate through state + persistence** — `SongState.sampleRate` (default 48000), `StateAPI::get/setSongSampleRate`, `songs.sample_rate` column. Tests: default/set + save-load round-trip.
- ✅ **2. `DeviceRateBridge` pure component** (`src/engine/DeviceRateBridge.h`) — async SRC (per-channel WindowedSinc + leftover buffer, `EngineRateSource` pull), pass-through when rates match, RT-safe. 6 unit tests: frequency preservation (48→44.1, 48→16 SCO, 44.1→48), continuity under ragged block sizes, reset. NOT yet wired into the graph.
- ✅ **3. Request engine rate at the device + Settings control** — `AudioEngine::setEngineSampleRate`/`applyEngineRate` (requests rate via `setAudioDeviceSetup`; graph still preps at the device's *actual* rate → safe fallback to device rate when refused; re-asserted on device change). Coordinator drives it from `syncTempoFromState`. Settings "Sample Rate" now sets the per-project engine rate. Click-tested: device follows, persists.
- ✅ **3b. `AudioFileNode` file→engine resampling** (reordered ahead of 4) — `processBlock` branches on `fileSampleRate/outputRate`: equal → original bit-exact 1:1; differ → stateless 4-point Catmull-Rom cubic from the beat-derived fractional position (continuous across blocks, seeks jump, fades preserved). Cubic not WindowedSinc because this node is random-access/beat-anchored, not a continuous stream. **This is what fixed the reported distortion** — a 44.1k import now plays clean in a 48k project and across engine-rate flips. Confirmed by ear + 3 unit tests (matched / 44.1→48 / 48→44.1).
- ✅ **4. Device-boundary SRC via `RateBridgeProcessor`** — the player drives the graph through the bridge (`AudioEngine::attachProcessor`); the graph runs at the engine rate, the bridge resamples device↔engine when they differ (output pull-driven, input best-effort, MIDI positions scaled). Passthrough when equal → byte-identical to before on every 48k rig. Fixes SCO Bluetooth (graph stays 48k, only the final output drops to 16k). 3 unit tests + app-launch smoke test (passthrough confirmed by ear; the built-in came up at 44.1k, the engine requested 48k, the device followed).
- ✅ **5. Import + bounce at engine rate; recordings store at engine rate automatically** — import/bounce use `getEngineSampleRate()`; post-phase-4 the record tap is downstream of the engine-rate graph, so takes are engine-rate with no extra work.
- ✅ **6. Bounce at engine rate** (`OfflineRenderer` renders at the engine rate, self-restoring). Input/output **latency compensation** is NOT part of this refactor — it stays a separate backlog item (see the built-in-mic / audio-latency notes), since it's orthogonal to the sample-rate boundary.

**Status: the sample-rate / device-boundary refactor is functionally complete** on branch `audio-device-boundary`. The Bluetooth wrong-rate bug is fixed (3b made mixed-rate playback correct at any rate; phase 4 keeps the graph at the engine rate so plugins/synths hold quality through a sub-48k output device). All device≠engine paths are unit-tested (the hardware can't produce that split, so no click-test); passthrough (device==engine, every rig Will uses) is byte-identical to before and confirmed by ear. Full suite green at 285. **Remaining before merge:** decide whether to merge to `main` (nothing outstanding for the bug); latency compensation remains a separate, deferred item.
...
- [x] **Audio assets browser** — *shipped phase 1 (`010247a`).* Tall/skinny **Assets** pane (Right slot, ⌘B) listing the song's distinct audio files (dedup by path across `regions` + `loops` **and** the loose-file registry) grouped by origin — Recorded / Imported / Other; rows are drag sources (mini-waveform + name + duration + mono/stereo). `TakeState.origin` set at record-commit + import (persisted). **Drag an asset → Looper track** = becomes that track's loop, defining the cycle if none exists; **→ Producer timeline** = places an audio region at the drop beat (`LooperPane`/`ProducePane` are `DragAndDropTarget`s; audio tracks only). **Loose-file registry** (`SongState.audioAssets` + `audio_assets` table + `StateAPI::add/removeAudioAsset`) holds files that exist in the project but aren't on a track — the lightweight alternative to the deferred full ref-counted registry. **Bounce Cycle** (cycle-mode-only button in the Producer transport group; enabled only for a single selected audio region the cycle overlaps): `exportRegionCycleChunk` slices the region's file to the cycle window → new WAV → registered loose asset. Reads the cycle from the sequencer loop range (not stale `song->cycle`) and persists it. **Still deferred:** the first-class `AudioAsset` registry (file-owned identity + ref-counting) and a **Bounces** category. See `memory/project_audio_assets_browser.md`.
...
**Build info** sits centered along the bottom edge of the sidebar (selectable/copyable TextEditor — same widget that used to live in the deleted top toolbar). Hidden when the sidebar collapses. See "Build identity" above.

### Navigation model

Every pane is a simple show/hide toggle — no modes, no workspace concept. A mode-switcher experiment (toolbar pill with per-mode layout state) was tried and reverted — the complexity wasn't justified for what's really just one pane swapping. There is **no top toolbar** — the app window is sidebar + dual-pane area + bottom mixer, with no app-level chrome above.
...
- **Audio region playback breaks when output device sample rate changes (Bluetooth).** *In progress* — being fixed as the engine sample-rate / device-boundary refactor (branch `audio-device-boundary`, phases 1–3 landed). Root cause confirmed in code: `AudioFileNode::processBlock` (`src/engine/AudioFileNode.h:82-97`) reads at the take's *stored* rate and copies file frames 1:1 to the output, with no resampling to the current device rate — so a device switching to 44.1k (A2DP) or 16k (SCO) garbles 48k content. The full fix (per-project engine rate + device-boundary async SRC + store-at-engine-rate) supersedes the originally-logged tactical "resampler inside AudioFileNode" idea. See the "Engine sample-rate / device-boundary refactor" sub-project note above and `docs/AUDIO_DEVICE_BOUNDARY.md`.
- **Audio latency, both directions** (~80ms peak-to-peak measured visually + audibly in the Producer). 2026-05-03 investigation outcomes:
  - **Focusrite (or any half-decent USB interface) = fine.** Latency is clean; recording lines up with the click. So Joe's first ship is unaffected — he records bass through an interface.
  - **Built-in Mac audio = ~80ms** even though Logic on the same hardware doesn't have this problem. So it's not a fundamental built-in-audio limitation; we're doing something Logic isn't.
  - **Settings → Audio buffer-size dropdown lies.** Config requested 128 samples; runtime fell back to 512 (built-in audio's effective minimum via the JUCE path); UI still showed 128 selected with no fallback indication. The dropdown reads `device->getCurrentBufferSizeSamples()` correctly but the user-visible state is misleading either way (looks like 128 is honored when it isn't).
  - **Possible deeper causes (unconfirmed):**
    - Aggregate-device construction when input-device ≠ output-device (CoreAudio creates an aggregate, which adds latency). Logic finds the unified built-in device.
    - JUCE's `AudioDeviceManager` may not be requesting the lowest IOBufferFrameSize via CoreAudio HAL the way Logic does (Logic uses AUHAL with explicit `kAudioDevicePropertyBufferFrameSize` writes).
    - `AudioProcessorGraph` may be inserting delay-compensation buffers if any plugin in the chain reports `getLatencySamples() > 0`.
    - We don't apply input-latency compensation on recording commit (Logic shifts captured audio backward by `device->getInputLatencyInSamples()`); even a perfect device would show some click-vs-recorded misalignment without that.
  - **Status: shelved post-Joe-ship** (2026-05-03). Not chasing the deeper investigation until we have a tester who actually trips on it. When we do return: add a diagnostic that logs `getInputLatencyInSamples + getOutputLatencyInSamples` per device callback, and start with the input-latency-compensation fix since it'd help every config — interface or built-in.
...
- **Producer ↔ Looper tempo coupling.** Tempo (and time signature) live on `SongState`, so changing tempo in the Producer changes the Looper too — and vice versa. Already-recorded loops drift relative to a freshly chosen tempo because their audio was captured at the old rate. For 0.1.0 we ship as-is and document it; longer-term the looper probably wants either its own tempo (separate from the song) or a re-stretch on tempo change. Revisit when a tester actually trips on it.
...
- **MainLayout** — sidebar + dual-pane area + mixer. No top toolbar (build info lives at the sidebar bottom).
- **ProducePane** — DAW arrange view: pane title ("Producer") centered above the track-headers column; transport button group anchored to the right edge of that column (matches Looper's anchor); LCD position center-aligned on full pane width; snap-to-grid + events-track toggles in the View group; two-row track headers (name / M/S/R/I pills), timeline grid, regions (MIDI piano roll or audio waveform), action track. Paint flows through a derived visual model (`Audibility` enum + `TrackRowVisuals` / `RegionVisuals` structs + `paintTrackRow` / `paintRegionShell` helpers) — add new visual axes there, not in scattered conditionals. Row bg goes through `TrackUi::rowBgForTrack()` (shared with Mixer + Looper). Region ops via Cmd/Shift selection + drag/keyboard. Auto-scroll, two-finger horizontal scroll. Keyboard: space/r/return, h/l (step by div), Shift+H/L (step by measure), Cmd+h/l/j/k (zoom).
- **LooperPane** — live-looping surface sharing the Left slot with Produce (mutually exclusive). Pane title ("Looper") centered above the track-headers column; segmented `BindableButton` strip anchored at the column edge (play | up down | replace overdub | undo redo mute clear | reset, uniform 84px cells); per-track rows with M/S/I pills + a piano-roll/waveform lane that paints in-flight content during capture. Lane background is `bgSlot` rather than the Producer's `bgSurfaceRaised` — track type is conveyed by the row's left-edge stripe. Shared row bg with Produce/Mixer via `TrackUi`. See `docs/LIVE_LOOPING.md` and `docs/LIVE_INPUT_AND_FOCUS.md`.
- **BindableButton** — segmented-strip cell wrapping a coordinator action. Two stacked layers: label/icon on top, plugin-slot-style "Trigger" affordance on the bottom (click → menu of every registered control across all devices, already-bound disabled, "Manage controls" jumps to Perform pane). Whole-cell flash on action fire (200ms `triggerLight` overlay at 35% alpha). Uniform across the Looper top bar; reusable by other panes that want bindable buttons.
...
- **AssetsPane** — Right-slot audio-file browser (⌘B), grouped by origin, drag source for placing audio into the Looper/Producer. Enumerates take files (`regions`+`loops`) plus `SongState.audioAssets` (loose files). Retries on a timer while any file's peaks are still computing on load. See the Backlog "Audio assets browser" entry + `memory/project_audio_assets_browser.md`.
...
Built-in actions include the automation set (`setActiveTrack`, `fadeOut`, `fadeIn`, `crossfade`, `trackVolume`, `morphToPreset`, `morphChain`, `morph`), transport (`togglePlay`), focus (`focusPrevTrack`, `focusNextTrack`, `toggleFocusedMute`), and the looper gestures (`replaceLoop`, `overdubLoop`, `undoLoop`, `redoLoop`, `clearLoop`, `clearAllLoops`, `resetLooperSession`). SongRuntime dispatches with wildcard fallback (exact → any device → any channel → any/any). The Looper top-bar `BindableButton` cells fire these via `coord.executeAction(...)`; bind hardware to them through each cell's per-button trigger slot.
