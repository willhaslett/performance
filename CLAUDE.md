...
- `docs/AI_COMPOSITION_API.md` — ABC notation + content CRUD redesign. **Shipped — merged to main 2026-05-16 (`0da51cd`).** Read before touching the composer pipeline, ABC parser/writer, RegionContent, or `runtime/SYSTEM_PROMPT.md` composing section.
- `docs/DAW_BRIDGE_PLAN.md` — forward-looking DAW bridge design.
- `docs/OUT_OF_PROCESS_HOSTING.md` — **active sub-project.** Crash isolation: running AU plugins out-of-process so a plugin segfault can't kill the host. Research-grounded reframing (AUv3 already isolated by JUCE 8; AUv2 — the plugins that crash us — load in-process). Read before touching plugin instantiation in `AudioEngine.mm` or the audio-thread render path. Includes threading teaching sections.
...
**AI composition redesign — shipped (merged to main 2026-05-16, `0da51cd`).** See `docs/AI_COMPOSITION.md` (research + landing) and `docs/AI_COMPOSITION_API.md` (build plan + deferral notes). The motivating problem: chat-composed music was "childish and sometimes plainly bad" (Will's words). The fix that landed: an **ABC notation + content-CRUD layer** replacing V2 notation — project / track / region exposed as ABC, tempo / time-sig / key as first-class multi-event CRUD resources via the unified `createEvent` API, `createAction` → `defineAction`, and the whole V2 surface (`compose`, `V2NotationParser`, `ComposerWriter`, `NotationParser`) deleted. 252 → 267 tests.

**Two open tails on the composition work:**
- **Mid-playback tempo (deferred engine sub-project).** Tempo-change events partway through a piece persist and are visible to the LLM, but the audio engine still plays the whole piece at `event[0].bpm`. Honoring them during transport is its own future engine sub-project.
- **Track-preset-first creation (named pre-merge blocker, merged unresolved).** Per `memory/project_track_preset_first.md`: asking chat to "create a synth track" yields a piano labeled "Synth" (DLSMusicDevice + default patch). The fix — `listTrackPresets()` / `applyTrackPreset()` Lua verbs plus a SYSTEM_PROMPT decision rule (prefer a user-saved track preset over a fresh plugin) — still does **not** exist in `LuaEngine.cpp`. Decide whether to close it before the next tester round.

Friend round still in wait-and-see; tester feedback light.
...
- **Factory theme JSONs out of sync with `Theme.h`.** On startup the app logs a wall of `[Theme] Unknown color key: bgRowActive/Muted/Selected/Focused` + `[Theme] Unknown key: headerHeight/slotHeight/pillSize/cornerRadius/...` warnings — the `runtime/themes/*.json` factory files (minimal_dark, minimal_light) don't define every token the code now reads, so those fall back to compiled `Theme.h` defaults. Harmless (fallback is intended), but the JSONs are stale. Cleanup: regenerate the factory JSONs from the current token set, and decide whether a missing key should warn at all given fallback is by-design. Surfaced during the 2026-06-13 smoke test.
- **Benign JUCE XmlElement assert on plugin-cache load.** `JUCE Assertion failure in juce_XmlElement.cpp:485` fires while parsing `~/.config/performance/plugin-cache.xml` at startup (and in the test run). Almost certainly an empty/whitespace text node in the cached XML — no functional impact. Worth tracking down so the assert stops firing, otherwise a real XML problem could hide behind a known-noisy assert. Surfaced during the 2026-06-13 smoke test.
